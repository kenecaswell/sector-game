...
/**
 * Buys `itemId` for `player` if it exists, they can afford it, and (for one-per-player items)
 ...
 *
 * Effects:
 *  - ammo:     + AMMO_PACK_SIZE shots (no cap yet). Buyable without a gun.
 *  - basicGun: the player can shoot (gun = 'basic').
...
function purchase(player: Player, itemId: unknown): boolean {
    ...
    if (itemId === 'basicGun' && player.gun !== '') return false; // already armed

    player.credits -= item.cost;
    if (itemId === 'ammo') {
        player.ammo += AMMO_PACK_SIZE;
    } else if (itemId === 'basicGun') {
        player.gun = 'basic';
    } else {
