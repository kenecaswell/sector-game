 ...
 * Runs only during `claiming`/`combat` (the phases where tiles are ownable) —
 * payouts stop once `results` begins so the economy can't change after the
 * match is decided, and `lobby` has no owned tiles yet regardless.
 *
...
function update(state: GameState): void {
  ...
  if (phase !== 'claiming' && phase !== 'combat') return;
