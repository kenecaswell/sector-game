...
- `CollisionSystem.claimTiles` runs each tick during `playing`. A player claims the hex they're standing on plus every hex whose center is within `Player.claimRadius`, found with a small search window around the player rather than a scan of the whole map. The radius is `BASE_CLAIM_RADIUS = HEX_SIZE` (32 world px; neighbor centers are ~55 px away, so in the open that's just the hex underfoot) or, with the Expander, `EXPANDER_CLAIM_RADIUS` = 4 × `PLAYER_RADIUS` = 80 px (it was 64 px when the player radius was 16). `CharacterSystem.applyUpgradeEffects` sets `claimRadius` from the player's upgrades.
- **Skipped hexes:** a teammate's hex (`areAllies`, 2026-09-26), and any hex in an enemy structure's 7-hex footprint (`isProtectedFrom`; a teammate's structure doesn't block you, but its tiles are a teammate's anyway). The protection keeps the invariant "you own the tiles your structure is on", which a large claim radius would otherwise break constantly.
...
**Teams.** `TEAMS` (in `shared/types.ts`) holds the 8 colors. `Player.teamId` holds one and `Player.color` is always that team's color, so everything that already drew in the player's color (tiles, body, structures, claim ring) shows the team with no rendering changes. There's no `GameState.teams` map: a team has no state of its own yet. `LobbySystem.defaultTeam` gives a newcomer the first team nobody is on yet, else the smallest.
...
- **Upgrades:** `Player.upgrades` lists `UpgradeId`s — `'boost'`, `'armor'` and `'expander'` (recorded as an upgrade since 2026-09-26; it used to be inferred from `claimRadius`). `CharacterSystem.applyUpgradeEffects` derives `maxHealth` and `claimRadius` from the list whenever it changes; the boost is read directly by `MovementSystem`.
...
- **Drawing (placeholder until art):** a raised slab (`STRUCTURE_HEIGHT` 14 screen px). The top face uses the type's color from `STRUCTURE_COLORS` in `client/src/game/constants.ts`; the sides and a 3 px border use the owner's team color. It's one `Graphics` per structure, drawn once. It sorts by its northmost corner, so its owner (or a teammate) standing on it draws on top instead of being hidden (they were, when it sorted by its center).
- **Build preview:** in build mode the hover outline becomes the structure hexagon, yellow if `canPlace` would accept it, red if not. A click on a red spot does nothing and stays in build mode; on touch (no hover) a refused tap shows the red outline for 1.2 s. The Build button's armed label is "Pick a spot — all 7 hexes must be yours". Build mode has no timeout (the old 5 s auto-disarm was removed 2026-09-26); `B`, the Build button and `Esc` are handled in `GameScreen`.
...
- **Damage comes from the shooter's gun** (`GUN_DAMAGE` in `shared/types.ts`). It's stamped on the projectile when fired (`Projectile.damage`), so it applies to players and structures alike. Health is `BASE_MAX_HEALTH` or `ARMOR_MAX_HEALTH`; respawns restore `maxHealth`. **The two guns' shots look different** (client, 2026-09-26, told apart by the synced `Projectile.damage`): see `BASIC_SHOT_*` / `BIG_SHOT_*` in `client/src/game/constants.ts`. Only the drawing differs; the hit radius is the same.
...
- **Upgrades:** `ShopSystem.purchase` adds the id to `Player.upgrades` and re-runs `applyUpgradeEffects`. For Armor it also adds the rise in `maxHealth` to current `health`, so a hurt player keeps their damage but gains the headroom.
