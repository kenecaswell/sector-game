...
const NORMAL_CLOSE_CODE = 1000; // consented room.leave() — do not attempt to reconnect
...
function bindMessageHandlers(room: GameRoom, handlers: GameEventHandlers): void {
  ...
  if (handlers.onTilesClaimed) room.onMessage<TilesClaimedEvent>('tilesClaimed', handlers.onTilesClaimed);
  if (handlers.onStructureDestroyed)
    room.onMessage<StructureDestroyedEvent>('structureDestroyed', handlers.onStructureDestroyed);
  if (handlers.onPhaseChanged) room.onMessage<PhaseChangedEvent>('phaseChanged', handlers.onPhaseChanged);
  if (handlers.onPlayerDisconnected)
    room.onMessage<PlayerDisconnectedEvent>('playerDisconnected', handlers.onPlayerDisconnected);
...
export function sendPlaceStructure(room: GameRoom, tileX: number, tileY: number, seq: number): void {
